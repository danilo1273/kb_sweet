-- Fix Production Scaling Logic (Batch Size)
-- Calculates planned quantity as: BOM_QTY * (ORDER_QTY / BATCH_SIZE)
-- Example: Recipe is for 500g. User wants 500g. Scaling Factor = 1.
-- Prevents the "multiplying by 500" error.
...
BEGIN
    ...
    -- Safety check for bad data (batch_size = 0)
    ...
    SELECT 
        ...
        -- (ItemQtyInRecipe * TargetProductionQty) / BatchSize
    ...
    RETURN jsonb_build_object(
        ...
        'scaling_factor', (p_quantity / v_batch_size) -- Debug info
    );
END;
$function$;
