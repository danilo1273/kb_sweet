-- 1. Ensure 'detail_order_id' exists in 'financial_movements'
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'financial_movements' AND column_name = 'detail_order_id') THEN
        ALTER TABLE financial_movements ADD COLUMN detail_order_id UUID REFERENCES sales(id);
    END IF;
END $$;

-- 2. Ensure 'location_id' exists in 'sales'
DO $$
BEGIN
...
-- 3. Backfill location_id based on stock_source (Legacy Migration)
DO $$
DECLARE
    r RECORD;
    loc_id UUID;
...
-- 4. Update process_sale RPC (Robust Version)
...
BEGIN
  ...
  LOOP
    ...
    IF v_current_stock IS NULL THEN
        -- Insert Key if missing (Should generally exist, but safe fallback)
    ...
    -- Update Legacy Columns on Products Table (Legacy Mirroring)
...
-- 5. Update delete_sale_secure RPC (Robust Version)
...
BEGIN
  ...
  -- 2. Delete Details (Safe Deletion)
  -- The WHERE clause now works because we ensured the column exists
