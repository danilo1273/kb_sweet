...
export default function Recipes() {
    ...
    return (
        <div className="flex-1 p-8 space-y-6 bg-[#FCFAF5] dark:bg-[#100F0D] min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    ...
                    Object.entries(filteredProducts.reduce((acc, product) => {
                        ...
                        .map(([category, items]) => (
                            <div key={category} className="col-span-1 md:col-span-2 lg:col-span-3 relative bg-[#FCF8F0] dark:bg-[#1C1A16] rounded-2xl border border-[#EDE5D6] dark:border-[#2D2821] shadow-sm p-6 pt-10 mb-8 overflow-hidden">
                                
                                {/* Caderno Binder Rings/Spiral Effect */}
                                <div className="absolute top-0 left-6 right-6 h-5 flex justify-between pointer-events-none opacity-90 select-none">
                                    {[...Array(12)].map((_, i) => (
                                        <div key={i} className="flex flex-col items-center">
                                            {/* Furo no papel */}
                                            <div className="w-2.5 h-2.5 rounded-full bg-zinc-200/80 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 shadow-inner"></div>
                                            {/* Anel de metal/arame */}
                                            <div className="w-1.5 h-7 bg-gradient-to-b from-zinc-300 via-zinc-200 to-zinc-400 dark:from-zinc-700 dark:via-zinc-600 dark:to-zinc-800 rounded-full -mt-1.5 shadow-sm border-r border-zinc-400 dark:border-zinc-900"></div>
                                        </div>
                                    ))}
                                </div>

                                <div className="pt-2 space-y-5">
                                    {/* Titulo de Categoria - Estilo Escrita de Livro */}
                                    <div className="flex items-center gap-2 border-b-2 border-dashed border-[#E5DAC4] dark:border-[#383127] pb-3">
                                        <span className="bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 p-2 rounded-xl shadow-sm border border-amber-200/30">
                                            <BookOpen className="h-4 w-4" />
                                        </span>
                                        <h3 className="text-xl font-bold font-serif text-[#534636] dark:text-[#DFD0BE] tracking-tight">{category}</h3>
                                        <span className="text-xs font-semibold text-[#837361] dark:text-[#A1907D] bg-[#EFEBDF] dark:bg-[#2B2721] px-2.5 py-0.5 rounded-full ml-auto border border-[#DECDB3] dark:border-[#3B342B]">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {items.map((product) => (
                                            <div key={product.id} className="group relative bg-white dark:bg-zinc-900 border border-[#EBE3D5] dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 flex flex-col min-h-[310px]">
                                                {/* Caderno Red Margin Line */}
                                                <div className="absolute top-0 left-3.5 bottom-0 w-px bg-red-200 dark:bg-red-900/50 pointer-events-none"></div>

                                                <div className="absolute top-4 right-4 z-10">
                                                    {product.type === 'intermediate' && (
                                                        <span className="bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300 text-[9px] font-bold px-2 py-0.5 rounded-md border border-amber-200 dark:border-amber-900 tracking-wider">
                                                            BASE
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                <div className="relative pt-4 px-4 pb-2 bg-white dark:bg-zinc-900 mx-4 mt-4 rounded-xl border border-zinc-100 dark:border-zinc-800 shadow-sm rotate-1 group-hover:rotate-0 transition-transform duration-300">
                                                    {/* Translucent Washi Tape Effect */}
                                                    <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-12 h-4.5 bg-amber-100/50 dark:bg-amber-950/40 border border-amber-200/20 backdrop-blur-[1px] rotate-[-4deg] shadow-sm pointer-events-none"></div>

                                                    <div className="h-32 bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center relative overflow-hidden rounded-lg shadow-inner">
                                                        {product.image_url ? (
                                                        ...
                                                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                                            <Button variant="secondary" size="sm" onClick={() => openEdit(product)} className="h-7 text-xs font-semibold bg-white text-zinc-950 hover:bg-zinc-100">
                                                                <Edit className="h-3 w-3 mr-1" /> Editar
                                                            </Button>
                                                            <Button variant="destructive" size="sm" onClick={() => handleDelete(product.id)} className="h-7 w-7 p-0 bg-red-600 hover:bg-red-700 text-white border-0">
                                                ...
                                                <div className="p-4 pt-2 flex flex-col flex-1 pl-6">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex-1 min-w-0 mr-2">
                                                            <h3 className="font-bold text-sm text-[#4E3F30] dark:text-[#E8DCCF] font-serif leading-tight group-hover:text-amber-800 dark:group-hover:text-amber-400 transition-colors truncate" title={product.name}>
                                                                {product.name}
                                                            </h3>
                                                            <div className="flex gap-1.5 mt-1.5 flex-wrap">
                                                                <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#F5EFE4] dark:bg-[#2B2721] text-[#7C6C58] dark:text-[#A89885] border border-[#EDE2CE] dark:border-[#38322A] uppercase tracking-wider">
                                                                ...
                                                                <span className={cn("text-[9px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider",
                                                                    (product.product_stocks?.reduce((acc: number, s: any) => acc + (Number(s.quantity) || 0), 0) || 0) > 0
                                                                        ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-emerald-950"
                                                                        : "bg-rose-50 dark:bg-rose-950/30 text-rose-700 dark:text-rose-400 border-rose-100 dark:border-rose-950")}>
                                                                    Estoque: {(product.product_stocks?.reduce((acc: number, s: any) => acc + (Number(s.quantity) || 0), 0) || 0)} {product.unit || 'un'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        {product.type === 'finished' && (
                                                            <span className="font-extrabold text-green-700 dark:text-green-400 text-sm whitespace-nowrap bg-green-50 dark:bg-green-950/30 border border-green-100 dark:border-green-950 px-2 py-0.5 rounded-lg shadow-sm">
                                                                R$ {product.price.toFixed(2)}
                                                            </span>
                                                        )}
                                                    </div>

                                                    <div className="mt-auto pt-3 border-t border-[#F0EBE0] dark:border-zinc-800 text-[10px] text-zinc-400 flex justify-between items-center">
                                                        ...
                                                        <div className="flex items-center gap-1 max-w-[60%] justify-end truncate">
                                                            {product.allowed_users && product.allowed_users.length > 0 ? (
                                                                <span className="text-amber-800 dark:text-amber-400 font-semibold truncate bg-amber-50 dark:bg-amber-950/20 px-2 py-0.5 rounded border border-amber-100/50 dark:border-amber-900/30"
                                                                ...
                                                                <span className="text-zinc-500 dark:text-zinc-400 font-medium truncate" title="Criador por (Legado)">
                                                                    👤 {allUsers.find(u => u.id === product.created_by)?.full_name?.split(' ')[0] || 'Criador'}
                                                                </span>
                                                            ) : (
                                                                <span className="text-zinc-400 dark:text-zinc-500 flex items-center gap-0.5 bg-zinc-50 dark:bg-zinc-950 px-2 py-0.5 rounded border">
                                                                    📖 Pública
                                                                </span>
                                                            )}
                                                        </div>
